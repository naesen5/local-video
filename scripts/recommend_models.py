...
def recommend_models(hardware_report):
    ...
    compatible_models.sort(
        key=lambda x: (not x.get("recommended", False), x["min_ram_gb"])
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Model recommendation for local video generation"
    )
    ...
    if args.hardware:
        ...
        hw_file = (
            Path.home() / ".openclaw" / "homework" / "state" / "hardware-report.json"
        )
        if hw_file.exists():
            ...
            hardware_report = {"ram": {"total_gb": 16}, "vram": {"gpus": []}}
